...
class CreateList(object):
    ...
    def print_event(cls, f: TextIO, epoch1: Epoch, satellite: int, type1: str, type2: str, type3: str,
        ...
        if row_counter == 0:  # print header
            f.write("\t \\resizebox{0.3\\textwidth}{!}{\n")
    ...
    def print_list(self, year) -> None:
        ...
        :rtype: None
        """
        f: TextIO = open("C:/Users/sophi/Desktop/Temp/myfile.tex", "w")
        ...
        for month in range(1, 6):
            ...
            self.print_page_header(f, epoch_start)
            f.write("\\begin{multicols}{3}\n")

            calc = Calculation(epoch_start, epoch_stop, calc_time_step, 0.0)

            print(len(calc.sorted_timings))
            x = len(calc.sorted_timings)
            for pos in range(0, x):
                row = calc.sorted_timings[pos].row
                epoch = calc.sorted_timings[pos].epoch
                col = calc.sorted_timings[pos].col
                ...
                if calc.sorted_timings[pos].appereance_type == 'start':
                    type2 = "D"
                else:
                    type2 = "F"
                type3 = "---"
