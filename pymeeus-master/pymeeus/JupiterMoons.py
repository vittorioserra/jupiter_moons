...
class JupiterMoons(object):
    ...
    def round_base_cone_param(epoch, ellipsoid=False):
        ...
        :returns: distance of the cone's sharpest point in jupiter-radii, always behind jupiter if viewed from the sun
        :rtype: float
        :raises: TypeError if input value if of wrong type
    ...
    @staticmethod
    def check_penumbra_eclipse(X, Y, Z, beta_cone_rad):
        """This method checks if the given coordinates correspond with a satellite
        phenomena. It returns if the satellite with the given coordinates is hidden
        behind Jupiter or directly in front.
