...
class TestJupiterMoons(TestCase):
    ...
    def test_rectangular_positions(self):
        ...
        assert abs(round(europe_corr_true[1], exp_prec) - round(0.27524463, exp_prec)) < TOL, \
            """ERROR: 2nd rectangular position for (Y) Europe of JupiterMoons.rectangular_position()
            test doesn't match"""

        assert abs(round(europe_corr_true[2], exp_prec) - round(-5.74710440, exp_prec)) < TOL, \
            """ERROR: 3rd rectangular position for (Z) Europe of JupiterMoons.rectangular_position()
            test doesn't match"""

        assert abs(round(ganymede_corr_true[0], exp_prec) - round(1.20111168, exp_prec)) < TOL, \
            """ERROR: 1st rectangular position (X) for Ganymede of JupiterMoons.rectangular_position()
            test doesn't match"""

        assert abs(round(ganymede_corr_true[1], exp_prec) - round(0.58999033, exp_prec)) < TOL, \
            """ERROR: 2nd rectangular position (Y) for Ganymede of JupiterMoons.rectangular_position()
            test doesn't match"""

        assert abs(round(ganymede_corr_true[2], exp_prec) - round(-14.94058137, exp_prec)) < TOL, \
    ...
    def test_correct_rectangular_positions(self):
        ...
        """ calculate corrected rectangular positions for the Jupiter moons Io"""
        i_sat = 1
        io = JupiterMoons.correct_rectangular_positions(R, i_sat, DELTA, X_coordinate, Y_coordinate, Z_coordinate)

        assert abs(round(io[0], exp_prec) - round(-3.450168811390241, exp_prec)) < TOL, \
            """ERROR: correction of 1st rectangular position (X) for Io test doesn't match"""
        assert abs(round(io[1], exp_prec) - round(0.21370246960509387, exp_prec)) < TOL, \
            """ERROR: correction of 2nd rectangular position (Y) for Io test doesn't match"""
        assert abs(round(io[2], exp_prec) - round(-4.818966623735296, exp_prec)) < TOL, \
            """ERROR: correction of 3rd rectangular position (Z) for Io test doesn't match"""

    def test_check_coordinates(self):
        """This method tests if the method check_coordinates() returns the right distance between Io and Jupiter
    ...
    def test_check_phenomena(self):
        """This method tests if the method check_phenomena() returns the right perspective distances
