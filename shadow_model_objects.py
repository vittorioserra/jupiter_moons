...
def test():
    ...
    io_shader = ShaderClass(io_radius, io_radius, 10**-2, defaultSolver)
    io_shaded = ShadedClass(io_radius, n_points, 0, delta, jupiter, io_shader)

    # configure the epoch to the moment in which the first event of 2020 for io is taking place
    epoch = epoch + 4831*s_jd*12 + 930*s_jd*60*2+1290*s_jd
    ...
    # the following returns an array containing the shadow cast by jupiter on the selected jovian moon
    for t in range(0, n_sec):

        # set epochs and get coordinates
        epoch = epoch + s_jd  # advance in 1s steps
        satellite_positions = np.array(JupiterMoons.rectangular_positions_jovian_equatorial(epoch))
        ...
        # compute the jovian shadow on Io and the quantify of light reflected toward earth
        ...
        if t%3600 == 0 :
            print(str(t/3600) + " hours simulated ")

    # the following section is kind of tricky since I have been dealing with a lot of numerical instability here
    # hence i try to filter the numerical instability out using a two stage 6 taps fir filter for each pixel

    # depth of the filter
    filter_depth = 12

    # correct the epoch for the filter
    epoch.set(2020, 1, 1, 0)
    epoch = epoch + 4831*s_jd*12 + 930*s_jd*60*2 +1290*s_jd

    sun_illumination_array_unfiltered = np.zeros([n_points, n_points, n_sec])
    ...
    for t in range(0, n_sec):
        ...
        # the sun
        sun_illumination_array_unfiltered[:, :, t] = io_shaded.ComputeReflectedLight(io_pos, sun_pos_eq_jvc, 1)
    ...
    illumination_array = np.clip((jovian_shadow_array[:, :, 0:(np.shape(jovian_shadow_array)[2]-14)] + (1-sun_illumination_array_filtered)), 0, 1)
    '''
    ...
    illumination_array = np.clip((jovian_shadow_array[:, :, 0:(np.shape(jovian_shadow_array)[2])] + (1-sun_illumination_array_filtered)), 0, 1)
    illumination_array = illumination_array * reflected_light_array[:, :, 0:(np.shape(jovian_shadow_array)[2])]
    ...
    #fig_2d_2 = px.imshow(jovian_shadow_array[:, :, :], animation_frame=2)
    #fig_2d_2.update_layout(title="Jovian shadow on Io")
    #fig_2d_2.show()

    sum_array = np.apply_over_axes(np.sum, illumination_array, [0, 1])
    sum_array = np.reshape(sum_array, [n_sec, ])

    fig_2_1 = px.line(y=sum_array, x=np.linspace(1, n_sec, n_sec))
    fig_2_1.update_layout(title= "Shadow cast by jupiter on Io and shadow caused by the sun")
    fig_2_1.show()

    '''
    sum_array = np.apply_over_axes(np.sum, jovian_shadow_array, [0,1]) #jovian shadow array
    sum_array = np.reshape(sum_array, [1500,])

    print(np.shape(sum_array))

    fig_2 = px.line(y=sum_array, x=np.linspace(1, n_sec, n_sec))
    fig_2.update_layout(title= "Shadow cast by jupiter on Io")
    fig_2.show()

    sum_array_sun = np.apply_over_axes(np.sum, sun_illumination_array_filtered* reflected_light_array[:, :, 0:(np.shape(jovian_shadow_array)[2]-14)], [0, 1])  # jovian shadow array
    sum_array_sun = np.reshape(sum_array_sun, [n_sec-14, ])

    print(np.shape(sum_array))

    final_light_array = np.apply_over_axes(np.sum, illumination_array, [0, 1])  # jovian shadow array
    final_light_array = np.reshape(final_light_array, [n_sec - 14, ])

    print(np.shape(sum_array))

    fig_2 = px.line(y=final_light_array, x=np.linspace(1, n_sec - 14, n_sec - 14))
